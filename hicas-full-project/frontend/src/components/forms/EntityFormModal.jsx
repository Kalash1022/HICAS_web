      ...
      <section className={`entity-modal ${sizeClass}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
        ...
        <form onSubmit={(event) => event.preventDefault()}>
          ...
          <footer className="modal-footer">
            {unavailableNotice ? <span className="form-unavailable-note" role="status">{unavailableNotice}</span> : null}
            <button className="secondary-button" type="button" onClick={() => navigate(returnPath)}>Hủy</button>
            <PrimaryButton type="submit" disabled={submitDisabled}>{submitLabel}</PrimaryButton>
          </footer>
        </form>
      </section>
    </main>
