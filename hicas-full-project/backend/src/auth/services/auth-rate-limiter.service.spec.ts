...
describe(AuthRateLimiterService.name, () => {
  ...
  it('limits avatar processing by both authenticated user and IP address', () => {
    for (let attempt = 0; attempt < 5; attempt += 1) {
      limiter.consumeAvatarUpload('customer-id', '127.0.0.9');
    }

    expect(() => limiter.consumeAvatarUpload('customer-id', '127.0.0.10')).toThrow(
      ApplicationException,
    );
    expect(() => limiter.consumeAvatarUpload('another-customer-id', '127.0.0.9')).not.toThrow();
  });
