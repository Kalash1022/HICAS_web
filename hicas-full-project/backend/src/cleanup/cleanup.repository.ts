...
export class CleanupRepository {
  ...
  async findUserAvatarStorageKeys(keys: string[]): Promise<Set<string>> {
    if (keys.length === 0) {
      return new Set();
    }
    const users = await this.database.user.findMany({
      where: { avatarStorageKey: { in: keys } },
      select: { avatarStorageKey: true },
    });
    return new Set(
      users.flatMap((user) => (user.avatarStorageKey === null ? [] : [user.avatarStorageKey])),
    );
  }
