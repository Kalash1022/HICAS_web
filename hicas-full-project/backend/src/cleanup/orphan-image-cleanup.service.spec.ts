...
const referencedAvatarId = '55555555-5555-4555-8555-555555555555';
const orphanAvatarId = '66666666-6666-4666-8666-666666666666';
const referencedAvatarKey = `users/${userId}/avatar/${referencedAvatarId}.webp`;
const orphanAvatarKey = `users/${userId}/avatar/${orphanAvatarId}.webp`;
...
describe(OrphanImageCleanupService.name, () => {
  ...
  beforeEach(() => {
    repository = {
      ...
      findUserAvatarStorageKeys: jest.fn(),
    ...
    repository.findUserAvatarStorageKeys.mockResolvedValue(new Set());
  });
  ...
  it('deletes only old, unreferenced, owned user avatar objects', async () => {
    storage.list.mockResolvedValueOnce({ objects: [] }).mockResolvedValueOnce({
      objects: [
        { key: referencedAvatarKey, lastModified: new Date(now.getTime() - 7_200_000) },
        { key: orphanAvatarKey, lastModified: new Date(now.getTime() - 7_200_000) },
        {
          key: `users/${userId}/avatar/77777777-7777-4777-8777-777777777777.webp`,
          lastModified: now,
        },
        {
          key: `users/${userId}/external-avatar.webp`,
          lastModified: new Date(now.getTime() - 7_200_000),
        },
      ],
    });
    repository.findUserAvatarStorageKeys
      .mockResolvedValueOnce(new Set([referencedAvatarKey]))
      .mockResolvedValueOnce(new Set());
    storage.delete.mockResolvedValue(undefined);

    await expect(service.cleanup(now)).resolves.toEqual({ inspected: 4, deleted: 1, skipped: 3 });

    expect(repository.findUserAvatarStorageKeys.mock.calls).toEqual([
      [[referencedAvatarKey, orphanAvatarKey]],
      [[orphanAvatarKey]],
    ]);
    expect(objectKeyLocks.runExclusive.mock.calls[0]?.[0]).toBe(orphanAvatarKey);
    expect(storage.delete.mock.calls).toEqual([[orphanAvatarKey]]);
  });
  ...
  it('moves through paginated object pages and restarts after the final page', async () => {
    storage.list
      .mockResolvedValueOnce({ objects: [], continuationToken: 'product-next-page' })
      .mockResolvedValueOnce({ objects: [], continuationToken: 'avatar-next-page' })
      .mockResolvedValueOnce({ objects: [] })
      .mockResolvedValueOnce({ objects: [] })
      .mockResolvedValueOnce({ objects: [] })
      .mockResolvedValueOnce({ objects: [] });
    ...
    expect(
      storage.list.mock.calls.map(([input]) => [input?.prefix, input?.continuationToken]),
    ).toEqual([
      ['products/', undefined],
      ['users/', undefined],
      ['products/', 'product-next-page'],
      ['users/', 'avatar-next-page'],
      ['products/', undefined],
      ['users/', undefined],
    ]);
  });
