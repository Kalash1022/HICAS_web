...
phone                     nullable
avatar_url                nullable
avatar_storage_key        nullable, unique; owned object key only
birth_date                nullable
...
3. `PATCH /api/v1/admin/products/:id` chỉ cho chuyển `DRAFT/ARCHIVED → ACTIVE` khi SKU/slug/category/price hợp lệ, Category active và có ít nhất một image; Inventory record phải tồn tại nhưng quantity có thể bằng 0. Khi Product đang `ACTIVE`, API từ chối xóa ảnh cuối cùng để không phá vỡ điều kiện public visibility.
...
- `users.avatar_url` có thể là URL bên ngoài từ Google. Chỉ `users.avatar_storage_key`
  khác `NULL` mới xác nhận object do hệ thống sở hữu và đủ điều kiện để cleanup/xóa.
